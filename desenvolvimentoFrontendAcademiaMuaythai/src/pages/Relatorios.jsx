...
export default function Relatorios() {
  // para dados gerais e filtros do painel
  ...
  // para retorna um array de anos para seleção (por exemplo, do ano atual-5 até o ano atual+1)
  ...
  useEffect(() => {
    ...
    // para gráfico: Alunos por Modalidade
    ...
    // para gráfico: Pagamentos por Status
    setGrafPagPorStatus([
      {
        name: "Pago",
        value: pagamentosLocal.filter((p) => p.status === "Pago").length,
      },
      {
        name: "Pendente",
        value: pagamentosLocal.filter((p) => p.status === "Pendente").length,
      },
    ]);

    // par gráfico: Valores por Mês (Pago x Pendente)
  ...
  // faz as funções de manipulação de pagamentos (todos os módulos de pagamentos)
  ...
  const gerarPDFPagamentosPendentes = () => {
    ...
    doc.text(
      `Total Pendente: R$ ${totalPendentes.toFixed(2)}`,
      20,
      doc.lastAutoTable.finalY + 10
    );
  ...
  const gerarPDFAluno = (aluno) => {
    ...
    let y = 30;
    doc.setFontSize(12);
    doc.text(`Nome: ${aluno.nome || "-"}`, 20, y);
    y += 10;
    doc.text(`Nascimento: ${aluno.nascimento || "-"}`, 20, y);
    y += 10;
    doc.text(`CPF: ${aluno.cpf || "-"}`, 20, y);
    y += 10;
    doc.text(`Endereço: ${aluno.endereco || "-"}`, 20, y);
    y += 10;
    doc.text(`Responsável: ${aluno.responsavel || "-"}`, 20, y);
    y += 10;
    doc.text(`Graduação: ${aluno.graduacao || "-"}`, 20, y);
    y += 10;
    doc.text(`Modalidade: ${aluno.modalidade || "-"}`, 20, y);
    y += 10;
    doc.text(
      `Mensalidade: R$ ${Number(aluno.valorMensalidade).toFixed(2)}`,
      20,
      y
    );
    y += 10;
    doc.text(`Forma de Pagamento: ${aluno.formaPagamento || "-"}`, 20, y);
    y += 10;
    const horariosStr =
      aluno.diasSelecionados && aluno.diasSelecionados.length > 0
        ? aluno.diasSelecionados
            .map((dia) => {
              const horarios =
                aluno.horariosSelecionados && aluno.horariosSelecionados[dia];
  ...
  const gerarPDFContasAPagar = () => {
    ...
    doc.text(
      `Pagas: R$ ${pagas.toFixed(2)}`,
      20,
      doc.lastAutoTable.finalY + 20
    );
    doc.text(
      `A Pagar: R$ ${aPagar.toFixed(2)}`,
      20,
  ...
  // para função central para gerar o relatório de contas a pagar agrupadas (por mês, semana ou ano)
  const handleGerarPDFContaRelatorio = () => {
    ...
    if (contaReportType === "geral") {
      ...
      // para iltra contas cujo vencimento (substring 0-7) corresponda a "YYYY-MM" e que tenham o status desejado
      filtered = contasAPagar.filter((c) => {
        if (!c.vencimento) return false;
        return (
          c.vencimento.substring(0, 7) === `${contaAno}-${contaPeriodo}` &&
      ...
      filtered = contasAPagar.filter((c) => {
        if (!c.vencimento) return false;
        return (
          c.vencimento.substring(0, 4) === String(contaAno) &&
          c.status.toLowerCase() === contaStatus
        );
      });
    }
    ...
    if (contaReportType === "mes") {
      title += ` - ${
        contaStatus === "pago" ? "Pagas" : "Pendentes"
      } - Mês: ${contaAno}-${contaPeriodo}`;
    } else if (contaReportType === "semana") {
      title += ` - ${
        contaStatus === "pago" ? "Pagas" : "Pendentes"
      } - Semana ${contaSemana} do Mês: ${contaAno}-${contaPeriodo}`;
    } else if (contaReportType === "ano") {
      title += ` - ${
        contaStatus === "pago" ? "Pagas" : "Pendentes"
      } - Ano: ${contaAno}`;
    ...
    doc.save(
      `relatorio_contas_${contaReportType}_${contaStatus}_${contaAno}_${contaPeriodo}_${contaSemana}.pdf`
    );
  };
  ...
  return (
    <div className="relatorios-container">
      ...
      <div className="menu-relatorios">
        <button
          onClick={() => setReportMode("resumo")}
          className={reportMode === "resumo" ? "active" : ""}
        >
          Resumo Geral
        </button>
        <button
        ...
        <button
          onClick={() => setReportMode("pagamentos")}
          className={reportMode === "pagamentos" ? "active" : ""}
        >
          Relatório de Pagamentos
        </button>
        <button
          onClick={() => setReportMode("contasAPagar")}
          className={reportMode === "contasAPagar" ? "active" : ""}
        >
                ...
                <ul>
                  {alunosData.map((aluno, i) => (
                    <li
                      ...
                      className={
                        selectedStudent && selectedStudent.cpf === aluno.cpf
                          ? "active"
                          : ""
                      }
                    >
            ...
            <div className="aluno-detalhe">
              {selectedStudent ? (
                <div className="detalhe-content">
                  ...
                  <p>
                    <strong>Nascimento:</strong> {selectedStudent.nascimento}
                  </p>
                  <p>
                    <strong>CPF:</strong> {selectedStudent.cpf}
                  </p>
                  <p>
                    <strong>Endereço:</strong> {selectedStudent.endereco}
                  </p>
                  <p>
                    <strong>Responsável:</strong> {selectedStudent.responsavel}
                  </p>
                  <p>
                    <strong>Graduação:</strong> {selectedStudent.graduacao}
                  </p>
                  <p>
                    <strong>Modalidade:</strong> {selectedStudent.modalidade}
                  </p>
                  <p>
                    <strong>Mensalidade:</strong> R${" "}
                    {Number(selectedStudent.valorMensalidade).toFixed(2)}
                  </p>
                  <p>
                    <strong>Forma de Pagamento:</strong>{" "}
                    {selectedStudent.formaPagamento}
                  </p>
                  <div className="detalhe-horarios">
                    <strong>Horários:</strong>
                    {selectedStudent.diasSelecionados &&
                    selectedStudent.diasSelecionados.length ? (
                      selectedStudent.diasSelecionados.map((dia) => {
                        const horarios =
                          selectedStudent.horariosSelecionados &&
                          selectedStudent.horariosSelecionados[dia];
                        return (
                          <div key={dia}>
                  ...
                  <button
                    className="btn-pdf"
                    onClick={() => gerarPDFAluno(selectedStudent)}
                  >
                    Exportar PDF
                  </button>
                </div>
        ...
        <div className="relatorio-contas-pagar">
          ...
          <div className="opcoes-relatorio">
            ...
            <div className="opcoes-buttons">
              <button
                onClick={() => setContaReportType("geral")}
                className={contaReportType === "geral" ? "active" : ""}
              >
                Geral
              </button>
              <button
                onClick={() => setContaReportType("mes")}
                className={contaReportType === "mes" ? "active" : ""}
              >
                Por Mês
              </button>
              <button
                onClick={() => setContaReportType("semana")}
                className={contaReportType === "semana" ? "active" : ""}
              >
                Por Semana
              </button>
              <button
                onClick={() => setContaReportType("ano")}
            ...
            <div className="opcoes-status">
              ...
              <label>
                <input
                  type="radio"
                  value="pago"
                  checked={contaStatus === "pago"}
                  onChange={(e) => setContaStatus(e.target.value)}
                />
                Pagas
              </label>
              <label>
            ...
            {contaReportType === "mes" && (
              <div className="opcoes-periodo">
                ...
                <select
                  value={contaPeriodo}
                  onChange={(e) => setContaPeriodo(e.target.value)}
                >
                ...
                <div className="opcoes-ano">
                  ...
                  <select
                    value={contaAno}
                    onChange={(e) => setContaAno(e.target.value)}
                  >
            ...
            {contaReportType === "semana" && (
              <div className="opcoes-periodo">
                ...
                <div className="opcoes-semana">
                  ...
                  <select
                    value={contaSemana}
                    onChange={(e) => setContaSemana(e.target.value)}
                  >
